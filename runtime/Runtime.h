...
class TypeArtRT final {
 public:
   ...
   *
   * \param[in] addr The address.
   * \return The id of the allocation, or -1.
   */
  int getReturnAddress(const void* addr) const;
  ...
  void onAlloc(const void* addr, int typeID, size_t count, int id);

  void onAllocStack(const void* addr, int typeID, size_t count, int id);

  void onAllocGlobal(const void* addr, int typeID, size_t count, int id);
 ...
 private:
  ...
  inline void doAlloc(const void* addr, int typeID, size_t count, int id, const char reg = 'H');
