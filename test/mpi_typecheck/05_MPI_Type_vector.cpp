...
int main(int argc, char** argv) {
  ...
  // RANK0: [Trace] Alloc 0x{{.*}} double 8 7
  auto too_small = new double[n - 1];
  if (rank == 0) {
    // clang-format off
    // RANK0: R[0][Info][1] MPI_Send: buffer 0x{{.*}} has type double, MPI type is double
    // RANK0: R[0][Error][1] MPI_Send: buffer 0x{{.*}} too small. The buffer can only hold 7 elements (8 required)
    // clang-format on
    MPI_Send(too_small, 1, mpi_double_vec, 1, 0, MPI_COMM_WORLD);
  } else {
    // clang-format off
    // RANK1: R[1][Info][0] MPI_Recv: buffer 0x{{.*}} has type double, MPI type is double
    // RANK1: R[1][Error][0] MPI_Recv: buffer 0x{{.*}} too small. The buffer can only hold 7 elements (8 required)
    // clang-format on
    MPI_Recv(too_small, 1, mpi_double_vec, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }
