...
void run(int rank) {
  // RANK0:  MPI_Send: internal error{{.*}} combiner MPI_Type_create_hindexed_block is currently not supported
  // RANK1: [Test] Received: 0 3 7
  if (rank == 0) {
    MPI_Datatype index_type;
    MPI_Aint displacements[3] = {0, 3 * sizeof(int), 7 * sizeof(int)};
    MPI_Type_create_hindexed_block(3, 1, displacements, MPI_INT, &index_type);
