...
class Domain {
 public:
  // Constructor
  Domain(Int_t numRanks, Index_t colLoc, Index_t rowLoc, Index_t planeLoc, Index_t nx, Int_t tp, Int_t nr,
  ...
  Real_t& x(Index_t idx) {
    return m_coord[idx].x;
  }
  Real_t& y(Index_t idx) {
    return m_coord[idx].y;
  }
  Real_t& z(Index_t idx) {
    return m_coord[idx].z;
  }

  // Nodal velocities
  Real_t& xd(Index_t idx) {
    return m_vel[idx].x;
  }
  Real_t& yd(Index_t idx) {
    return m_vel[idx].y;
  }
  Real_t& zd(Index_t idx) {
    return m_vel[idx].z;
  }

  // Nodal accelerations
  Real_t& xdd(Index_t idx) {
    return m_acc[idx].x;
  }
  Real_t& ydd(Index_t idx) {
    return m_acc[idx].y;
  }
  Real_t& zdd(Index_t idx) {
    return m_acc[idx].z;
  }

  // Nodal forces
  Real_t& fx(Index_t idx) {
    return m_force[idx].x;
  }
  Real_t& fy(Index_t idx) {
    return m_force[idx].y;
  }
  Real_t& fz(Index_t idx) {
    return m_force[idx].z;
  }

  // Nodal mass
  Real_t& nodalMass(Index_t idx) {
    return m_nodalMass[idx];
  }

  // Nodes on symmertry planes
  Index_t symmX(Index_t idx) {
    return m_symmX[idx];
  }
  Index_t symmY(Index_t idx) {
    return m_symmY[idx];
  }
  Index_t symmZ(Index_t idx) {
    return m_symmZ[idx];
  }
  bool symmXempty() {
    return m_symmX.empty();
  }
  bool symmYempty() {
    return m_symmY.empty();
  }
  bool symmZempty() {
    return m_symmZ.empty();
  }

  //
  // Element-centered
  //
  Index_t& regElemSize(Index_t idx) {
    return m_regElemSize[idx];
  }
  Index_t& regNumList(Index_t idx) {
    return m_regNumList[idx];
  }
  Index_t* regNumList() {
    return &m_regNumList[0];
  }
  ...
  // elem connectivities through face
  Index_t& lxim(Index_t idx) {
    return m_faceToElem[idx].lxim;
  }
  Index_t& lxip(Index_t idx) {
    return m_faceToElem[idx].lxip;
  }
  Index_t& letam(Index_t idx) {
    return m_faceToElem[idx].letam;
  }
  Index_t& letap(Index_t idx) {
    return m_faceToElem[idx].letap;
  }
  Index_t& lzetam(Index_t idx) {
    return m_faceToElem[idx].lzetam;
  }
  Index_t& lzetap(Index_t idx) {
    return m_faceToElem[idx].lzetap;
  }

  // elem face symm/free-surface flag
  Int_t& elemBC(Index_t idx) {
    return m_elemBC[idx];
  }

  // Principal strains - temporary
  Real_t& dxx(Index_t idx) {
    return m_dxx[idx];
  }
  Real_t& dyy(Index_t idx) {
    return m_dyy[idx];
  }
  Real_t& dzz(Index_t idx) {
    return m_dzz[idx];
  }

  // Velocity gradient - temporary
  Real_t& delv_xi(Index_t idx) {
    return m_delv_xi[idx];
  }
  Real_t& delv_eta(Index_t idx) {
    return m_delv_eta[idx];
  }
  Real_t& delv_zeta(Index_t idx) {
    return m_delv_zeta[idx];
  }

  // Position gradient - temporary
  Real_t& delx_xi(Index_t idx) {
    return m_delx_xi[idx];
  }
  Real_t& delx_eta(Index_t idx) {
    return m_delx_eta[idx];
  }
  Real_t& delx_zeta(Index_t idx) {
    return m_delx_zeta[idx];
  }

  // Energy
  Real_t& e(Index_t idx) {
    return m_e[idx];
  }

  // Pressure
  Real_t& p(Index_t idx) {
    return m_pq[idx].p;
  }

  // Artificial viscosity
  Real_t& q(Index_t idx) {
    return m_pq[idx].q;
  }

  // Linear term for q
  Real_t& ql(Index_t idx) {
    return m_qlqq[idx].ql;
  }
  // Quadratic term for q
  Real_t& qq(Index_t idx) {
    return m_qlqq[idx].qq;
  }

  Real_t& delv(Index_t idx) {
    return m_delv[idx];
  }

  // Relative volume
  Real_t& v(Index_t idx) {
    return m_vol[idx].v;
  }
  // Reference volume
  Real_t& volo(Index_t idx) {
    return m_vol[idx].volo;
  }

  // volume derivative over volume
  Real_t& vdov(Index_t idx) {
    return m_vdov[idx];
  }

  // Element characteristic length
  Real_t& arealg(Index_t idx) {
    return m_arealg[idx];
  }

  // Sound speed
  Real_t& ss(Index_t idx) {
    return m_ss[idx];
  }

  // Element mass
  Real_t& elemMass(Index_t idx) {
  ...
  Real_t monoq_max_slope() const {
    return m_monoq_max_slope;
  }
  Real_t monoq_limiter_mult() const {
    return m_monoq_limiter_mult;
  }
  Real_t ss4o3() const {
    return m_ss4o3;
  }
  Real_t qlc_monoq() const {
    return m_qlc_monoq;
  }
  Real_t qqc_monoq() const {
    return m_qqc_monoq;
  }
  Real_t qqc() const {
    return m_qqc;
  ...
  Real_t eosvmax() const {
    return m_eosvmax;
  }
  Real_t eosvmin() const {
    return m_eosvmin;
  }
  Real_t pmin() const {
    return m_pmin;
  ...
  Real_t dvovmax() const {
    return m_dvovmax;
  }
  Real_t refdens() const {
    return m_refdens;
  }
...
// lulesh-comm
void CommRecv(Domain& domain, Int_t msgType, Index_t xferFields, Index_t dx, Index_t dy, Index_t dz, bool doRecv,
              bool planeOnly);
void CommSend(Domain& domain, Int_t msgType, Index_t xferFields, Domain_member* fieldData, Index_t dx, Index_t dy,
              Index_t dz, bool doSend, bool planeOnly);
