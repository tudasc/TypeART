...
LD_PRELOAD="$TYPEART_PATH_RT/libtypeartRuntime-d.so $TYPEART_PATH_INTERCEPT/libtypeartMPITool-d.so" TA_EXE_TARGET=$exe TA_TYPE_FILE=${type_file} mpiexec --oversubscribe -n $np $exe $args &> "$log_file"
