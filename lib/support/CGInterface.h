...
class CGInterface {
  public:
     ...
     */
    virtual bool reachable(const std::string &source, const std::string &target, bool case_sensitive = false, bool short_circuit = true) = 0;
...
class JSONCG : public CGInterface {
  public:
    JSONCG(const llvm::json::Value &cg);
    bool reachable(const std::string &source, const std::string &target, bool case_sensitive = false, bool short_circuit = true) override;
